...
/**
 * Modified by Kian Mistry on 14/01/17.
 */
 ...
 *
 * @param <M> The class type to use as a model for the data contained in the children of the
 *            given Firebase location
 */
public abstract class FirebaseRecyclerAdapter<M, VH extends ViewHolder> extends Adapter<VH> {

	private Query mQuery;
	private Class<M> mItemClass;
	 ...
	 */
	public FirebaseRecyclerAdapter(Query query, Class<M> itemClass) {
	 ...
	 */
	public FirebaseRecyclerAdapter(Query query, Class<M> itemClass, ArrayList<M> items, ArrayList<String> keys) {
		this.mQuery = query;
	...
	private ChildEventListener mListener = new ChildEventListener() {
		...
		public void onChildAdded(DataSnapshot dataSnapshot, String previousChildName) {
			...
			if (!mKeys.contains(key)) {
				M item = dataSnapshot.getValue(FirebaseRecyclerAdapter.this.mItemClass);
		...
		public void onChildChanged(DataSnapshot dataSnapshot, String s) {
			...
			if (mKeys.contains(key)) {
				...
				M oldItem = mItems.get(index);
				M newItem = dataSnapshot.getValue(FirebaseRecyclerAdapter.this.mItemClass);
	 ...
	 */
	protected abstract void itemAdded(M item, String key, int position);
	 ...
	 */
	protected abstract void itemChanged(M oldItem, M newItem, String key, int position);
	 ...
	 */
	protected abstract void itemRemoved(M item, String key, int position);
	 ...
	 */
	protected abstract void itemMoved(M item, String key, int oldPosition, int newPosition);
}